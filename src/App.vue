<template>
  <nav class="border-b border-pink-900">
    <div class="bg-pink-600 h-24">
      <div class="Logo-part flex">
        <svg
          class="ml-5 mt-4"
          height="40px"
        ...
        <h1 class="text-white text-2xl ml-2 mt-5">The To-Do List</h1>
      </div>
      <h3 class="ml-16 text-pink-300">Your own digital To-Do list</h3>
    </div>
  </nav>

  <div class="transform mt-16 flex justify-center items-center">
    <div class="search-bar">
      <input
        placeholder="Add new item"
        class="bg-yellow-100 rounded-xl w-96 px-8 py-0.5 h-16 text-lg"
        v-model="text_note"
        @keyup.enter="addNote"
      />
      <div class="absolute top-5">
        <i class="fa-solid fa-plus text-gray-500 ml-3"></i>
      </div>
    </div>
    <i
      class="
        ...
        p-2
        text-gray-500
        border border-dashed border-yellow-100
        rounded-md
        ml-2
      "
      @click="toggle()"
  ...
  <div class="flex justify-center h-full">
    <div
      ...
      "
    >
      <h2 class="font-bold text-lg underline text-yellow-300 mt-2">
        To be completed:
      </h2>
      <ul>
        <li
          class="text-md font-bold text-gray-800 py-1"
          v-for="item in normal_notes"
          :key="item"
        >
          <i
            class="fa-solid fa-circle-check px-1"
            :class="item.is_finished ? 'text-green-700' : ''"
            @click="finishNote(item.id)"
          ></i
          >{{ item.note }}
          <i
            class="px-1 fa-solid fa-circle-minus text-red-700"
    ...
    <div
      ...
      "
    >
      <h2 class="font-bold text-lg underline text-green-400 mt-2">
        Completed:
      </h2>
      <ul>
        <li
          class="text-md font-bold text-gray-800 py-1"
          v-for="item in completed_notes"
          :key="item"
        >
          <i
            class="fa-solid fa-circle-check px-1"
    ...
    >
      <div class="flex items-end">
        <h2 class="font-bold text-lg underline text-red-600 mt-2">Deleted:</h2>
        <h3
          class="
            text-gray-500
            hover:text-gray-700
...
export default {
  ...
  setup() {
    ...
    function getAllNotes() {
      //call backend
      axios.get("http://127.0.0.1:5173/api/note").then((backend_notes) => {
        this.items = backend_notes.data;
      });
    }

    function clearBin() {
      for (let i = 0; i < this.items.length; i++) {
        if(this.items[i].is_deleted == true)
        {
        axios
          .delete(
            "http://127.0.0.1:5173/api/remove?id=" + this.items[i].id
          )
          .then((response) => {
    ...
    function removeNote(id) {
      ...
      axios
        .put("http://127.0.0.1:5173/api/update/" + id, {
          note: note.note,
    ...
    function finishNote(id) {
      ...
      for (let i = 0; i < this.items.length; i++) {
        if (this.items[i].id == id) {
          this.items[i].is_finished = !this.items[i].is_finished;
          note = this.items[i];
          break;
        }
      }
    ...
    function addNote() {
      ...
      // });
      axios
        .post("http://127.0.0.1:5173/api/add", {
          note: this.text_note,
          is_finished: false,
          is_deleted: false,
        })
  ...
  computed: {
    normal_notes() {
      return this.items.filter(
        (el) => el.is_finished == false && el.is_deleted == false
      );
    },
    completed_notes() {
      return this.items.filter(
        (el) => el.is_finished == true && el.is_deleted == false
      );
    },
    deleted_notes() {
      return this.items.filter((el) => el.is_deleted == true);
    },
  },
